...
-- TODO: on release key, update variables such as `walkLeft`
    ...
    | c == 'd'  = World { worldInfo = worldInfo { player = (player worldInfo) { playerVel = (v, 0) } }, .. }
    where v = 10
handleKeys _ World{..} = World { worldInfo = worldInfo { player = (player worldInfo) { playerVel = (0, 0) } }, .. }
...
movePlayer seconds World{..} = World { worldInfo = worldInfo { player = (player worldInfo) { playerLoc = (x', y') } }, .. }
  where
